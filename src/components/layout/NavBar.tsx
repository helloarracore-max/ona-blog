...
import { Sparkles, Sun, Moon, X, Menu, PenTool, LogOut, Heart } from 'lucide-react';
        ...
        <nav className="w-full fixed top-0 z-50 pt-4 px-4 sm:pt-6 sm:px-8">
            <div className={`relative flex max-w-7xl mx-auto rounded-full px-6 py-3 items-center justify-between transition-all duration-300 border ${isDark ? 'bg-neutral-900/80 border-white/10' : 'bg-white/80 border-black/5'} backdrop-blur-xl shadow-lg z-50`}>
                ...
                <div className="hidden md:flex items-center gap-8 text-sm font-medium text-neutral-400">
                    <button onClick={() => handleNavigate('/')} className={`transition-colors hover:text-fuchsia-400 ${isDark ? 'text-neutral-400' : 'text-neutral-600'}`}>Roots</button>
                    <button onClick={() => handleNavigate('/?category=travel')} className={`transition-colors hover:text-fuchsia-400 ${isDark ? 'text-neutral-400' : 'text-neutral-600'}`}>Travel</button>
                    <button onClick={() => handleNavigate('/?category=style')} className={`transition-colors hover:text-fuchsia-400 ${isDark ? 'text-neutral-400' : 'text-neutral-600'}`}>Style</button>
                    <button onClick={() => handleNavigate('/?category=growth')} className={`transition-colors hover:text-fuchsia-400 ${isDark ? 'text-neutral-400' : 'text-neutral-600'}`}>Growth</button>
                </div>
                    ...
                    <div className="absolute top-20 left-4 right-4 md:hidden">
                        <div className={`rounded-3xl p-6 border shadow-2xl backdrop-blur-2xl animate-in slide-in-from-top-4 fade-in duration-200 ${isDark ? 'bg-neutral-900/95 border-white/10' : 'bg-white/95 border-black/5'}`}>
                            <div className="flex flex-col gap-4">
                                <button onClick={() => handleNavigate('/')} className={`text-left text-lg font-medium p-2 rounded-xl ${isDark ? 'text-neutral-300 hover:bg-white/5' : 'text-neutral-600 hover:bg-black/5'}`}>Roots (Home)</button>
                                <button onClick={() => handleNavigate('/?category=travel')} className={`text-left text-lg font-medium p-2 rounded-xl ${isDark ? 'text-neutral-300 hover:bg-white/5' : 'text-neutral-600 hover:bg-black/5'}`}>Travel Logs</button>
                                <button onClick={() => handleNavigate('/?category=style')} className={`text-left text-lg font-medium p-2 rounded-xl ${isDark ? 'text-neutral-300 hover:bg-white/5' : 'text-neutral-600 hover:bg-black/5'}`}>Style Edits</button>
                                <button onClick={() => handleNavigate('/?category=growth')} className={`text-left text-lg font-medium p-2 rounded-xl ${isDark ? 'text-neutral-300 hover:bg-white/5' : 'text-neutral-600 hover:bg-black/5'}`}>Growth Journal</button>
